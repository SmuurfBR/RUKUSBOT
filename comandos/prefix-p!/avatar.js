    ...
    if (!razaou.length < 1) {
        ...
        if (message.mentions.users.size < 1) {
            ...
            if(client.users.get(args[0])){
                ...
                if(client.guilds.get(message.guild.id).members.filter(mem=>{mem.displayName != message.content.replace("r!avatar ", "")})){
                    message.channel.sendMessage({
                        "embed": {
                          "description": `:frame_photo: **Avatar de ${client.guilds.get(message.guild.id).members.filter(mem=>{mem.displayName != message.content.replace("r!avatar ", "")}).user.username}:**`,
                          "color": 55512,
                          "timestamp": new Date(),
                          "footer": {
                            "icon_url": message.author.displayAvatarURL,
                          ...
                          "image": {
                            "url": client.guilds.get(message.guild.id).members.filter(mem=>{mem.displayName != message.content.replace("r!avatar ", "")}).user.displayAvatarURL
                          }
                        }
                      });
                } else {
